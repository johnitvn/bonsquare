...
dotenv.config({ path: join(__dirname, '../../../web-api-gateway/.env') });
...
module.exports = async function () {
  ...
  if (process.env.BASE_URL) {
    ...
    await exec('npx', ['nx', 'start', 'web-api-gateway']);
    console.log('Dev server started');

    console.log(`Test will run on base url: http://${DEFAULT_SERVER_HOST}:${DEFAULT_SERVER_PORT}`);
    globalThis.__BASE_URL__ = `http://${DEFAULT_SERVER_HOST}:${DEFAULT_SERVER_PORT}`;
  } else {
