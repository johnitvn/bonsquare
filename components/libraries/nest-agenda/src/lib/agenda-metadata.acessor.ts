...
import { AGENDA_MODULE_DEFINITION, AGENDA_MODULE_PROCESSOR, AGENDA_MODULE_SCHEDULE } from './agenda.constants';
...
export class AgendaMetadataAccessor {
  ...
  getDefinitionOptions(target: Function) {
    return this.reflector.get<AgendaDefinitionOptions>(AGENDA_MODULE_DEFINITION, target);
  }

  getScheduleOptions(target: Function) {
    return this.reflector.get<AgendaScheduleOptions>(AGENDA_MODULE_SCHEDULE, target);
  }
}
