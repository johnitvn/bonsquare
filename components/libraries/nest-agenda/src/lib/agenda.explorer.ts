    ...
    providers
      ...
      .forEach((wrapper) => {
        ...
        this.metadataScanner.scanFromPrototype(instance, Object.getPrototypeOf(instance), (key: string) => {
          ...
          const processor = (...args: unknown[]) => methodRef.call(instance, ...args);

          const definitionOpts = this.metadataAcessor.getDefinitionOptions(methodRef);
          if (!definitionOpts) return;
          const { name, options } = definitionOpts;
          this.logger.debug(`Registering processor "${name}"`);
          agendaService.define(name, options, processor);

          const scheduleOptions = this.metadataAcessor.getScheduleOptions(methodRef);
          if (!scheduleOptions) return;
          const { when, type } = scheduleOptions;
          this.logger.debug(`Scheduling processor "${name}" of type "${type}" ${when}`);

          if (type === AgendaScheduleType.Every) agendaService.every(<string>when, name);
