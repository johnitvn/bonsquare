 ...
 * HTML `<link>` tag. These tags define document linkdata that is important for
 ...
 *
 * @see [HTML link tag](https://developer.mozilla.org/docs/Web/HTML/Element/link)
...
export class Link {
  ...
  updateTag(tag: LinkDefinition, selector?: string): HTMLLinkElement | null {
    ...
    if (meta) {
      return this._setLinkElementAttributes(tag, meta);
    }
  ...
  private _getOrCreateElement(meta: LinkDefinition, forceCreation = false): HTMLLinkElement {
    if (!forceCreation) {
    ...
    this._setLinkElementAttributes(meta, element);
